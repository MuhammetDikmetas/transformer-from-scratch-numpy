...
print("son loss:", round(float(loss), 4))

def decode(ids):
    return "".join(itos[int(i)] for i in ids)


def generate(prompt, n_new=200, temperature=0.8, top_k=5):
    ids = [stoi[c] for c in prompt]

    for _ in range(n_new):
        ctx = np.array(ids[-block_size:], dtype=np.int64)
        T = len(ctx)
        c, s = rope_tables(T, d_head)

        X = E[ctx]
        for blk in blocks:
            X, _ = block_forward(X, blk, c, s)
        Xn, _ = rmsnorm_forward(X, g_f)
        logits = Xn @ Wl

        logit = logits[-1] / temperature

        if top_k is not None:
            keep = np.argsort(logit)[-top_k:]
            filtered = np.full_like(logit, -np.inf)
            filtered[keep] = logit[keep]
            logit = filtered

        probs = softmax(logit[None, :])[0]
...
print("\n[temperature=0.5]")
print(generate("dikkat mekanizmasi", n_new=200, temperature=0.5))

print("\n[temperature=1.0]")
print(generate("dikkat mekanizmasi", n_new=200, temperature=1.0))